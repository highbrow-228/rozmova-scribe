...
[ -z "$1" ] && { echo "usage: ./run-wav.sh <file.wav> [model] [tag]"; exit 1; }
...
chan=$(ffprobe -v error -select_streams a:0 -show_entries stream=channels -of csv=p=0 "$1")
log "формат: ${rate}Hz, каналів: $chan"

if [ "$rate" = "16000" ] && [ "$chan" = "1" ]; then
  log "вже 16kHz моно, конвертація не потрібна"
  src="$1"
else
  log "конвертую у 16kHz моно"
  tmp="$BASE/${name}_16k.wav"
  if [ "$rate" -le 8000 ]; then
    log "телефонна якість, нормалізацію гучності пропускаю"
    filt="highpass=f=80"
  else
    filt="highpass=f=80,loudnorm=I=-16:TP=-1.5:LRA=11"
  fi
  ffmpeg -y -loglevel warning -stats -i "$1" -vn \
    -af "$filt" \
    -ac 1 -ar 16000 -c:a pcm_s16le "$tmp"
  src="$tmp"
  log "готово: $(du -h "$tmp" | cut -f1)"
fi
